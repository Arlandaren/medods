...
func main(){
	cfg, err := config.Get()
	if err != nil{
		fmt.Println("ошибка переменнэх окружения:", err)
		return
	}
	...
	if err != nil{
		fmt.Println("ошибка подкючения к бд:", err)
		return
	}
	defer pool.Close()
	...
	if err != nil{
		fmt.Println("ошибка инициализации сервисов:", err)
		return
	}
	...
	err = r.Run(cfg.Addr)
	if err != nil{
		fmt.Println("ошибка инииализации роутов:",err)
		return
	}
